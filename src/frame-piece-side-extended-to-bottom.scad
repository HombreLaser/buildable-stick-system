...
module side_and_bottom_frame_piece_wall(lower=false) {
    ...
    projection(cut=true) rotate([0, 90, 0]) translate([frame_x/2-frame_bevel_height, 0, 0])
        side_frame_piece(lower=lower);
    projection(cut=true) translate([0, -frame_y/2+frame_wall, 0]) rotate([90, 90, 0]) bottom_piece_extension();
}

module side_and_bottom_frame_piece_extension(lower=false) {
	// combine the original with a shrunken piece to "pull" the wall out
	hull() {
		linear_extrude(height=frame_extension_y) scale([0.5, 0.90, 1]) side_and_bottom_frame_piece_wall(lower=lower);
		linear_extrude(height=frame_bevel_height) side_and_bottom_frame_piece_wall(lower=lower);
	}
}

module extra_extended_left_or_right_frame_piece(lower=false) {
	difference() {
		side_frame_piece(lower=lower);
	...
	translate([-frame_x/2+frame_bevel_height*2, 0, 0]) rotate([0, -90, 0]) side_and_bottom_frame_piece_extension(lower=lower);
}
